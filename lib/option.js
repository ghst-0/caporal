import { GetterSetter } from './utils.js';
import { Validator } from './validator.js';
import { OptionSyntaxError } from './error/option-syntax-error.js';
...
class Option extends GetterSetter {
  ...
  _analyseSynopsis() {
    ...
    const infos = this._synopsis.split(/[\s\t,]+/).reduce((acc, value) => {
      if (value.slice(0, 2) === '--') {
        acc.long = value;
        acc.longCleanName = value.slice(2);
        acc.allCleanNames.push(value.slice(2));
      } else if (value.slice(0, 1) === '-') {
        acc.short = value;
        acc.shortCleanName = value.slice(1);
        acc.allCleanNames.push(value.slice(1));
      } else if (value.slice(0, 1) === '[') {
