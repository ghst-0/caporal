...
import { UnknownOptionError } from './error/unknown-option.js';
import { InvalidOptionValueError } from './error/invalid-option-value.js';
import { InvalidArgumentValueError } from './error/invalid-argument-value.js';
import { MissingOptionError } from './error/missing-option.js';
import { NoActionError } from './error/no-action-error.js';
import { WrongNumberOfArgumentError } from './error/wrong-num-of-arg.js';
...
class Command extends GetterSetter {
   ...
   */
  args(index) {
    return index === undefined ? this._args : this._args[index];
  }
   ...
   */
  _getLongOptions() {
    return this._options.map(opt => opt.getLongCleanName()).filter(o => o !== undefined);
  }
   ...
   */
  command() {
    return this._program.command(...arguments);
  }
  ...
  _hasVariadicArguments() {
    return this.args().some(a => a.isVariadic()) !== undefined;
  }
  ...
  _checkRequiredOptions(options) {
    return this._options.reduce((acc, opt) => {
      if (acc[opt.getLongCleanName()] === undefined && acc[opt.getShortCleanName()] === undefined) {
        if (opt.hasDefault()) {
  ...
  _run(args, options) {
    ...
    try {
      const actionResults = this._action(args, options);
...
Object.defineProperties(Command, {
  "NATIVE_OPTIONS": {
    value: ['h', 'help', 'V', 'version', 'color', 'v', 'verbose']
  }
});
