import Table from '@alexbosworth/cli-table3';
import { colorize } from './colorful.js';
...
class Help {
  ...
  _getCommands() {
    ...
    for (const cmd of this._program._commands
      // don't include default command
      .filter((cmnd) => cmnd.name() !== '' && cmnd.visible())) {
        commandTable.push(
  ...
  _getGlobalOptions() {
    ...
    for (const o of this._getPredefinedOptions()) {
      optionsTable.push(o)
    }
  ...
  _renderHelp(key) {
    ...
    let rendered = '';
    for (const args of this._customHelp[key]) {
      const help = args[0];
